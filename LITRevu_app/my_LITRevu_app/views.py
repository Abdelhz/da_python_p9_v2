...
def feed(request):
    ...
    def get_users_viewable_tickets(user):
        ...
        for ticket in tickets:
            ticket.can_add_review = ticket.review_set.filter(user=user).exists()
        return tickets
    ...
    reviews_to_my_tickets = Review.objects.filter(ticket__user=request.user).exclude(user=request.user)
    # returns queryset of reviews made to tickets owned by the current user except reviews made by the current user to its own tickets
    reviews_followed = (reviews | reviews_to_my_tickets).distinct().order_by('-time_created')
    ...
    tickets_followed = tickets_followed.annotate(content_type=Value('TICKET', CharField()))
    ...
    context = {"user_tickets": user_tickets, "user_reviews": user_reviews, "reviews_followed": reviews_followed, "tickets_followed": tickets_followed, "user": request.user}
...
def view_ticket(request, ticket_id):
    ...
    my_reviews = Review.objects.filter(ticket=ticket, user=request.user).order_by('-time_created')
    other_reviews = Review.objects.filter(ticket=ticket).exclude(user=request.user).order_by('-time_created')
    context = {'ticket': ticket, 'my_reviews': my_reviews, 'other_reviews': other_reviews, 'user': request.user}
...
def add_review(request):
    if request.method == 'POST':
        ...
        try:
            ...
            if form_ticket.is_valid():
                ...
                logger.warning(f"TicketForm is not valid: {form_ticket.errors}")
                return render(request, 'my_LITRevu_app/reviews/add_review.html', {'form': form, 'form_ticket': form_ticket})
        ...
        if form_review.is_valid():
            ...
            review.save()
            return redirect('posts')  # Redirect to posts after successful review addition
        else:
    ...
    return render(request, 'my_LITRevu_app/reviews/add_review.html', {'form_review': form_review, 'form_ticket': form_ticket})


@login_required(login_url='login')
def view_review(request, review_id):
    review = get_object_or_404(Review, id=review_id)
    return render(request, 'my_LITRevu_app/reviews/view_review.html', {'review': review, 'user': request.user})
...
def add_review_to_ticket(request, ticket_id):
    ...
    if request.method == 'POST':
        ...
        form = ReviewForm()
    return render(request, 'my_LITRevu_app/reviews/add_review_to_ticket.html', {'form': form, 'ticket': ticket})
...
class EditTicketView(UpdateView):
    model = Ticket
    fields = ['title', 'description', 'image']  # list all the fields that you want to be editable
...
def subscriptions(request):
    ...
    blocked_users = UserBlock.objects.filter(user_blocking=request.user).select_related('user_blocked')
    blocked_list = [user_block.user_blocked for user_block in blocked_users]
    return render(request, 'my_LITRevu_app/subscriptions.html', {'following_list': following_list, 'blocked_list': blocked_list})

@login_required(login_url='login')
def search_user(request):
    query = request.GET.get('username')
    if query:
        # Find users whose username starts with the query
        users = User.objects.filter(Q(username__istartswith=query))
    else:
        users = User.objects.none()

    return render(request, 'my_LITRevu_app/search_results.html', {'users': users})
...
def search_user(request):
    ...
    following = UserFollows.objects.filter(user=request.user).select_related('followed_user')
    following_list = [user_follow.followed_user for user_follow in following]
    blocking = UserBlock.objects.filter(user_blocking=request.user).values_list('user_blocked', flat=True)
    blocked_by = UserBlock.objects.filter(user_blocked=request.user).values_list('user_blocking', flat=True)
    if query:
        users = User.objects.filter(Q(username__istartswith=query)).exclude(id=request.user.id)
    else:
        users = User.objects.none()

    return render(request, 'my_LITRevu_app/search_results.html', {'users': users, 'following_list': following_list, 'blocking': blocking, 'blocked_by': blocked_by})
...
def follow_user(request, user_id):
    ...
    if request.method == "POST":
        ...
        if UserBlock.objects.filter(user_blocking=request.user, user_blocked=user_to_follow).exists() or UserBlock.objects.filter(user_blocking=user_to_follow, user_blocked=request.user).exists():
            messages.error(request, "You can't follow this user because one of you has blocked the other.")
        ...
        try:
            ...
            messages.error(request, "You are already following {}.".format(user_to_follow.username))
        ...
        return HttpResponseRedirect(reverse('search_user') + '?username=' + query)
...
def unfollow_user(request, user_id):
    ...
    if request.method == "POST":
        ...
        logger.warning('source_page: %s', source_page)  # Log the source_page variable
...
def block_user(request, user_id):
    ...
    if request.method == "POST":
        ...
        UserBlock.objects.create(user_blocking=request.user, user_blocked=user_to_block)
...
def unblock_user(request, user_id):
    ...
    if request.method == "POST":
        ...
        UserBlock.objects.filter(user_blocking=request.user, user_blocked=user_to_unblock).delete()
